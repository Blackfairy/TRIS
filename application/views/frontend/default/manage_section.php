<div class="col-lg-5" style="padding: 35px;">
    <div class="content-box" id = "add_form_area">
        <form action="<?php echo site_url('home/sections/add/'.$manuscript_id); ?>" id = "ajaxFormForAdding" onsubmit="addSection(event)" method="post" enctype="multipart/form-data">
...
<div class="col-lg-7" style="padding: 35px;">
    <div class="content-box" id = "reload_section">
        <table class="table">
            ...
            <tbody>
                <?php
                    $sections = json_decode($manuscript_details['section'], true);
                    ...
                    <?php foreach ($sections as $section):
                        ...
                        <td>
                            ...
                            <a href="#" onclick="confirm_modal('<?php echo site_url('home/sections/delete/'.$manuscript_id.'/'.$section); ?>');"><?php echo get_phrase('delete'); ?></a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <input type="hidden" id = "ajax_url_for_serialization" name="" value="<?php echo site_url("home/sections/serialize_section/$manuscript_id"); ?>">
    </div>
    </div>
    ...
    <script type="text/javascript">
        function addSection(event) {
            ...
            jQuery(formData).each(function(i, item){
                var url = "<?php echo site_url('home/sections/add/'.$manuscript_id); ?>";
                $.post(url, {'title' : item.value, 'manuscript_id' : '<?php echo $manuscript_id; ?>'}, function(response){
        ...
        function updateSection(event) {
            ...
            event.preventDefault();
            jQuery(formData).each(function(i, item){
                var url = "<?php echo site_url('home/sections/edit/').$manuscript_id.'/'; ?>" + $('#section_id_for_editing').val();
                $.post(url, {'title' : item.value, 'manuscript_id' : '<?php echo $manuscript_id; ?>'}, function(response){
                    $('#reload_section').html(response);
                });
                //console.log(url);
