<?php
// $param2 = lesson id and $param3 = manuscript id
    $lesson_details = $this->crud_model->get_lessons('lesson', $param2)->row_array();
    $sections = $this->crud_model->get_section('manuscript', $param3)->result_array();
?>
...
<form action="<?php echo site_url('admin/lessons/'.$param3.'/edit'.'/'.$param2); ?>" method="post" enctype="multipart/form-data">
    ...
    <input type="hidden" name="manuscript_id" value="<?php echo $param3; ?>">
    ...
    <div class="form-group">
        <label for="section_id"><?php echo get_phrase('manuscript_type'); ?></label>
        <select class="form-control select2" data-toggle="select2" name="lesson_type" id="lesson_type" required onchange="show_lesson_type_form(this.value)">
            <option value=""><?php echo get_phrase('select_type_of_manuscript'); ?></option>
...
function ajax_get_section(manuscript_id) {
    $.ajax({
        url: '<?php echo site_url('admin/ajax_get_section/');?>' + manuscript_id ,
