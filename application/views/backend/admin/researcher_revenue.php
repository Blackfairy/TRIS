            ...
            <div class="card-body">
                ...
                <div class="table-responsive-sm mt-4">
                    <table class="table table-striped table-centered mb-0">
                        <thead>
                            <tr>
                                <th><?php echo get_phrase('accessed_manuscript'); ?></th>
                            ...
                            <?php foreach ($payment_history as $payment):
                                ...
                                <tr class="gradeU">
                                    <td>
                                        ...
                                        <small class="text-muted"><?php echo get_phrase('accessed_date').': '.date('D, d-M-Y', $payment['date_added']); ?></small>
                                    </td>
                                    ...
                                    <td style="text-align: center;">
                                        <?php if ($payment['researcher_payment_status'] == 0): ?>
                                            <?php if ($paypal_keys[0]['production_client_id'] != ""): ?>
                                                <form action="<?php echo site_url('admin/paypal_checkout_for_researcher_revenue'); ?>" method="post">
                                                    <input type="hidden" name="amount_to_pay"        value="<?php echo $payment['researcher_revenue']; ?>">
                                                    <input type="hidden" name="payment_id"           value="<?php echo $payment['id']; ?>">
                                                    <input type="hidden" name="researcher_name"      value="<?php echo $user_data['first_name'].' '.$user_data['last_name']; ?>">
                                                    <input type="hidden" name="manuscript_title"         value="<?php echo $manuscript_data['title']; ?>">
                                            ...
                                            <?php else: ?>
                                                <button type="button" class = "btn btn-outline-danger btn-sm btn-rounded" name="button" onclick="alert('<?php echo get_phrase('this_researcher_has_not_provided_valid_paypal_client_id'); ?>')"><?php echo get_phrase('pay_with_paypal'); ?></button>
                                            <?php endif; ?>
                                            <?php if ($stripe_keys[0]['public_live_key'] != "" && $stripe_keys[0]['secret_live_key']): ?>
                                                <form action="<?php echo site_url('admin/stripe_checkout_for_researcher_revenue'); ?>" method="post">
                                                    <input type="hidden" name="amount_to_pay"   value="<?php echo $payment['researcher_revenue']; ?>">
                                                    <input type="hidden" name="payment_id"      value="<?php echo $payment['id']; ?>">
                                                    <input type="hidden" name="researcher_name" value="<?php echo $user_data['first_name'].' '.$user_data['last_name']; ?>">
                                                    <input type="hidden" name="manuscript_title"    value="<?php echo $manuscript_data['title']; ?>">
                                            ...
                                            <?php else: ?>
                                                <button type="button" class = "btn btn-outline-danger btn-sm btn-rounded" name="button" onclick="alert('<?php echo get_phrase('this_researcher_has_not_provided_valid_public_key_or_secret_key'); ?>')"><?php echo get_phrase('pay_with_stripe'); ?></button>
                                            <?php endif; ?>
                                        <?php else: ?>
